      ...
      <div class="modal-wrapper">
        <div class="modal-container">
          <div class="modal-body" v-if="currStore">
            ...
            <div class="footer">
              <a href="#" @click.prevent="close()">關閉</a>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    currStore() {
      return this.$store.state.stores.filter((d) => d.id === this.infoboxId)[0];
    },
    servicePeriods() {
      let s = this.currStore?.service_periods || '';
      s = s.replace(/N/g, 'O').replace(/Y/g, 'X');
      return s ? [s.slice(0, 7).split(''), s.slice(7, 14).split(''), s.slice(14, 21).split('')] : s;
    },
  },
  methods: {
    close() {
